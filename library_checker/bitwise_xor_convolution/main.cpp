...
//   g(k) = ∑_i (-1)^|i∧k| f(i)
template<class T>
void WalshHadamard(vector<T>& f, bool inv) {
  int n = f.size();
  for (int l = 2; l <= n; l *= 2) {
    for (int k = 0; k < n; k += l) {
      for (int i = 0; i < l / 2; i++) {
...
void mainCase() {
  ...
  WalshHadamard(xs, false);
  WalshHadamard(ys, false);
  FOR(i, 0, n) { zs[i] = xs[i] * ys[i]; }
  WalshHadamard(zs, true);
...
/*
python misc/run.py library_checker/bitwise_xor_convolution/main.cpp
