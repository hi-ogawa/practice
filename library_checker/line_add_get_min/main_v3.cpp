...
struct LiChaoTree {
  ...
  struct Node {
    vec2 data = {0, inf};
    array<Node*, 2> to = {};
    ~Node() { for (auto ptr : to) { if (ptr) { delete ptr; } } }
  };
  ...
  LiChaoTree() : root_ptr{&root} {}

  void insert(vec2 f) {
    y_combinator([&](auto rec, T l, T r, Node*& node) -> void {
      if (!node) {
        node = new Node();
        node->data = f;
        return;
    ...
    })(root_l, root_r, root_ptr);
  }

  T evaluate(T x) {
    return y_combinator([&](auto rec, T l, T r, Node*& node) -> T {
      if (!node) { return inf; }
...
/*
python misc/run.py library_checker/line_add_get_min/main_v3.cpp
