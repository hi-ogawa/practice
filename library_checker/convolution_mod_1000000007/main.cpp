...
// Bezout identity: xp + yq = gcd(p, q)
constexpr tuple<ll, ll, ll> solveBezout(ll p, ll q) {
  bool _swap = 0;
...
void polymulCRT(vector<int>& p, vector<int>& q, vector<int>& r, int modulo) {
  ...
  auto solveCRT = [&](ll x1, ll x2) {
    x1 = (x1 * n2) % m1;
    x2 = (x2 * n1) % m2;
    return ((x1 * m2 + x2 * m1) % (m1 * m2)) % modulo;
  };
