...
void mainCase() {
  ...
  // Cum sum for row/column/diagonal free cels
  vector<vector<mint>> dp1(n + 1, vector<mint>(m + 1));
  vector<vector<mint>> dp2(n + 1, vector<mint>(m + 1));
  vector<vector<mint>> dp3(n + 1, vector<mint>(m + 1));
  ...
  FOR(i, 0, n) {
    FOR(j, 0, m) {
      ...
      dp[i][j] += dp2[i][j];
      dp[i][j] += dp3[i][j];
      dp1[i    ][j + 1] = dp1[i][j] + dp[i][j];
      dp2[i + 1][j    ] = dp2[i][j] + dp[i][j];
      dp3[i + 1][j + 1] = dp3[i][j] + dp[i][j];
    }
  }
