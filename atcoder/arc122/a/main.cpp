...
void mainCase() {
  ...
  // dp(k, 0) = #{ combinations of +/- with + at the left most }
  // dp(k, 1) = #{ combinations of +/- with - at the left most }
  ...
  mint res = (dp[n - 1][0] + dp[n - 1][1]) * ls[0];
  FOR(i, 1, n) {
    ll x = i - 1;
    ll y = n - i - 1;
    mint pos = (dp[x][0] + dp[x][1]) * (dp[y][0] + dp[y][1]);
    mint neg = dp[x][0] * dp[y][0];
    res += (pos - neg) * ls[i];
  }
  cout << res << "\n";
}
...
/*
python misc/run.py atcoder/arc122/a/main.cpp
