...
struct SegmentTree {
  ...
  mint reduce(int ql, int qr) {
    function<mint(int, int, int)> rec = [&](int l, int r, int j) {
...
void mainCase() {
  ...
  dp.set(0, 1);
  FOR(j, 3, s + 1) {
    dp.set(j, dp.reduce(0, j - 3 + 1));
  }
  auto res = dp.reduce(s, s + 1);
  cout << res << "\n";
}
...
/*
python misc/run.py atcoder/abc178/d/main_v2.cpp --check
