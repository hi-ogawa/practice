...
void mainCase() {
  ...
  for (auto [_0, _1, i, is_start]: events) {
    ...
    if (is_start) {
      ...
      pending.erase({x, i});
      if (pending.size() && get<0>(*pending.begin()) <= x) {
        contained[i] = 1;
      }
      if (finished.size() && get<0>(*finished.rbegin()) >= x) {
        contains[i] = 1;
      }
      finished.insert({x, i});
    }
...
/*
python misc/run.py cses/sorting_and_searching/task2168/main_v2.cpp
