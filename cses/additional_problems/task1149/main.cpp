...
// Main
void mainCase() {
  string s; // length \in [1, 5 x 10^5]
  cin >> s;

  vector<mint> dp(1 << 8);
  mint res = 1;
  for (auto c : s) {
    tie(res, dp[c]) = make_tuple(2 * res - dp[c], res);
  }
  res -= 1; // Subtract empty string
  cout << res << "\n";
}
...
/*
python misc/run.py cses/additional_problems/task1149/main.cpp
