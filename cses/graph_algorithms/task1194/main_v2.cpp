...
// NOTE: Cull goal by monster-free shortest path
...
void mainCase() {
  ...
  // Collect goals with "monster-free" shortest path (BFS from A)
  set<tuple<int, int>> goals;
  {
    vector<vector<int>> dists(n, vector<int>(m, -1));
    deque<tuple<int, int, bool>> queue; // (i, j, monster-free)
    auto [ia, ja] = A;
    queue.push_back({ia, ja, 1});
    dists[ia][ja] = 0;
    while (!queue.empty()) {
      auto [i, j, mfree] = queue.front(); queue.pop_front();
      if (board[i][j] == 'M') {
        mfree = 0;
      }
      if (i == 0 || i == n - 1 || j == 0 || j == m - 1) {
        if (mfree) { goals.insert({i, j}); }
        continue;
      }
...
/*
python misc/run.py cses/graph_algorithms/task1194/main_v2.cpp --check
