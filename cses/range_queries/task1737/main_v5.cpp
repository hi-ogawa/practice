...
// NOTE: Implement shared_ptr by ourselves. Clearly faster but you even need destructor so not to leak...
...
struct SegmentTree {
  ...
  struct Node {
    ...
    // Increase use count of children when copied
    Node(const Node& other) : cs{other.cs}, data{other.data} {
      FOR(i, 0, 2) {
        if (cs[i]) { cs[i]->use_count++; }
      }
    }
...
/*
python misc/run.py cses/range_queries/task1737/main_v5.cpp
