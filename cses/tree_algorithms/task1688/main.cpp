...
void mainCase() {
  ...
  FOR(i, 1, n) { adj[parents[i]].push_back(i); }

  // DFS for depth
  vector<int> depths(n, -1);
  function<void(int, int)> dfs = [&](int v, int d) {
    depths[v] = d;
    for (auto u : adj[v]) { dfs(u, d + 1); }
  };
  dfs(0, 0);
  ...
  // Lowest common ancestor by binary search
...
/*
python misc/run.py cses/tree_algorithms/task1688/main.cpp --check
