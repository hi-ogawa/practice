...
struct SegmentTree {
  ...
  SegmentTree(int n) : n_{n} {
    ...
    FOR(i, 0, 1 << depth_) { set(i, - i - 1); } // Initialize with irrelavant distinct elements
  }
  ...
  void set(int qi, int qv) {
    ...
    while (i > 0) {
      i = ((i + 1) / 2) - 1;
      auto [x1, y1, c1] = data_[2 * i + 1];
      auto [x2, y2, c2] = data_[2 * i + 2];
      data_[i] = {x1, y2, c1 + c2 + (y1 == x2)};
    }
  }

  void _convert(int src, int tgt, int i, int l, int r) {
    int size = r - l;
    auto [x, y, c] = data_[i];
    // DD(tie(i, l, r, c, size));

    // Bulk convert when segment contains single type
    // (this covers the case of leaf too)
    if (size - 1 == c) {
      int z = x == src ? tgt : x;
      data_[i] = {z, z, c};
      return;
    ...
    // Follow children and update as usual
    int mid = (l + r) / 2;
    int i0 = 2 * i + 1;
    int i1 = 2 * i + 2;
    _convert(src, tgt, i0, l, mid);
    _convert(src, tgt, i1, mid, r);
    auto [x1, y1, c1] = data_[i0];
    auto [x2, y2, c2] = data_[i1];
    data_[i] = {x1, y2, c1 + c2 + (y1 == x2)};
  }

  void convert(int src, int tgt) {
    _convert(src, tgt, 0, 0, 1 << depth_);
  }
...
/*
python misc/run.py codeforces/edu91/e/main_v2.cpp --check
