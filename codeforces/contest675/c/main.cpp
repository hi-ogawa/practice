...
void mainCase() {
  ...
  vector<mint> fs(n); // f(k) = 1x10^0 + 2x10^1 + 3x10^2 + ... + kx10^(k - 1)
  FOR(k, 1, n) {
    fs[k] = fs[k - 1] + k * mint(10).pow(k - 1);
  }

  mint res = 0;
  FOR(i, 0, n) {
    // Consider how many times "s[i]" gets added at which digit positions
    mint x = s[i] - '0';
    // Remove "higher segment" than "i" (thus digit position is 10^i)
    mint t1 = x * mint(10).pow(i) * mint::binom(n - i, 2);
    // Remove "lower segment" than "i" (thus digit position decrease e.g. 10^0, 10^1, ..., 10^(i-1))
    mint t2 = x * fs[i];
    res += t1 + t2;
  }
  cout << res << "\n";
...
/*
python misc/run.py codeforces/contest675/c/main.cpp --check
