...
// NOTE: #{nodes in LiChaoTree} ~ Q log2(2^31) ~ 10^7 and each node takes at least 32 bytes so 256MB is a bit too tight.
//       But, in principle, persistence implementation should be correct.
...
struct LiChaoTree {
  ...
  void rollback() {
    ...
    while ((int)history.size() > cp) {
      ...
      if (t == 1) { assert(*p1); p1->reset(nullptr); }
...
/*
python misc/run.py codeforces/problemset/678F/main.cpp
