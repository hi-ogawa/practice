...
// NOTE: Use "int" where "ll" is not necessary.
...
void mainCase() {
  ...
  const ll kInvalid = 1LL << 62;
  vector<ll> res(nq, kInvalid);
  ...
  FOR(i, 0, nq) {
    ...
    if (t == 3) {
      auto z = res[i];
      cout << (z == kInvalid ? "EMPTY SET" : to_string(z)) << "\n";
    }
  }
}
