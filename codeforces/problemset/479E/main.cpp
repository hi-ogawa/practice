...
void mainCase() {
  ...
  vector<mint> dp(b + 1), dp_diff(b + 1);
  dp[a] = 1;
  FOR(_, 0, k) {
    fill(ALL(dp_diff), 0);
    FOR(i, 0, b) {
       // Range increment by differece array
      int d = b - i - 1;
      int i0 = max(0, i - d), i1 = i + d + 1;
      dp_diff[i0]    += dp[i];
      dp_diff[i]     -= dp[i];
      dp_diff[i + 1] += dp[i];
      dp_diff[i1]    -= dp[i];
    ...
    // Apply increments
    dp = dp_diff;
    FOR(i, 1, b) { dp[i] += dp[i - 1]; }
  }

  mint res = accumulate(dp.begin(), dp.begin() + b, (mint)0);
  cout << res << "\n";
}
...
/*
python misc/run.py codeforces/problemset/479E/main.cpp --check
