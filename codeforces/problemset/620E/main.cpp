...
// Segment tree for range set and range sum
struct SegmentTree {
  int n = 1;
...
void mainCase() {
  ...
  for (auto [t, x, y] : qs) {
    ...
    if (t == 1) {
      FOR(j, 0, k) {
        // Set all descendents to all "0" or all "1"
        trees[j].set(i, i + s, j == y);
      }
    }
    if (t == 2) {
      int res = 0;
      FOR(j, 0, k) {
        // Check if any descendent is "1"
        int cnt = trees[j].reduce(i, i + s);
        res += cnt > 0;
      }
      cout << res << "\n";
    }
...
/*
python misc/run.py codeforces/problemset/620E/main.cpp
