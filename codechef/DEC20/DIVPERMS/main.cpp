...
void mainCase() {
  ...
  // dp(S, i, k) = #{ ways to pass "S" and end at "i" with score "k" }
  V<V<V<mint>>> dp(1 << n, V<V<mint>>(n, V<mint>(n)));
  FOR(i, 0, n) {
    dp[1 << i][i][0] = 1;
  }
  FOR(s, 0, 1 << n) {
    FOR(i, 0, n) {
      ...
      FOR(j, 0, n) {
        if (s & (1 << j)) { continue; }
        int ii = i + 1, jj = j + 1;
        int cost = ((jj % ii) == 0 && (ls[(jj / ii) - 1] - '0'));
        FOR(k, 0, n - 1) {
          dp[s | (1 << j)][j][k + cost] += dp[s][i][k];
        }
      }
    }
  }
...
/*
python misc/run.py codechef/DEC20/DIVPERMS/main.cpp
